...
def read_h5ad_minimal(h5ad_path):
    ...
    with h5py.File(h5ad_path, "r") as f:
        # Try CSR
        if "X" in f and all(k in f["X"] for k in ["data","indptr","indices","shape"]):
            data = f["X"]["data"][()]
            indptr = f["X"]["indptr"][()]
            indices = f["X"]["indices"][()]
        ...
        if "var" in f and "_index" in f["var"]:
            genes = [g.decode("utf-8") if isinstance(g, bytes) else str(g) for g in f["var"]["_index"][()]]
        elif "var_names" in f:
            genes = [g.decode("utf-8") if isinstance(g, bytes) else str(g) for g in f["var_names"][()]]
        else:
...
def main():
    ...
    if os.path.exists("expr.csv") and os.path.exists("genes.txt") and os.path.exists("pseudotime.csv"):
        print("[INFO] Found existing CSV inputs; skipping .h5ad conversion.")
        sys.exit(0)

    if args.h5ad is None or not os.path.exists(args.h5ad):
        print("[ERROR] No .h5ad found and no CSVs present. Provide filtered_placeholder.h5ad or expr.csv/genes.txt/pseudotime.csv.", file=sys.stderr)
        sys.exit(2)

    if not has_h5py():
        print("[ERROR] Python package 'h5py' not found. Install with: pip install h5py", file=sys.stderr)
        print("[TIP] Or provide CSVs manually: expr.csv, genes.txt, pseudotime.csv", file=sys.stderr)
        sys.exit(3)
