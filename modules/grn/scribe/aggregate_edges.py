...
if not edges.empty and not avg.empty:
    ...
    s = pd.to_numeric(df.get("score", pd.Series([0]*len(df))), errors="coerce").fillna(0.0)
    w = pd.to_numeric(df.get("weight", pd.Series([0]*len(df))), errors="coerce").fillna(0.0)
    if s.max() > 0:
        s = s / s.max()
    df["final_score"] = np.maximum(s.values, w.values)
    df = df[["regulator","target","final_score"]]
elif not avg.empty:
    df = avg.rename(columns={"weight":"final_score"})[["regulator","target","final_score"]]
else:
    df = edges.rename(columns={"score":"final_score"})[["regulator","target","final_score"]]

df = df[df["regulator"] != df["target"]].copy()
df = df.sort_values("final_score", ascending=False).drop_duplicates(["regulator","target"])
